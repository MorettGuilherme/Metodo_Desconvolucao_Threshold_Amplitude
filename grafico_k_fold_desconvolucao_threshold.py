...
# Objetivo do código: construção do gráfico da validação cruzada K-Fold para o método de Desconvolução de Sinal Versão Threshold.
...
titulo_programa = colored("Plote do gráfico da validação cruzada K-Fold para o método de Desconvolução de Sinal Versão Threshold:\n", attrs=["bold"])
...
def leitura_dados_estatisticos_k_fold(numero_ocupacao, dado_estatistico):
    ...
    pasta_dados_k_fold = f"K_Fold_{dado_estatistico}_Dados_Estatisticos_Desconvolucao_Threshold_OC"

    # Nome do arquivo de entrada dos dados estatísticos do K-Fold.
    arquivo_dados_k_fold = f"k_fold_{dado_estatistico}_dados_estatisticos_desconvolucao_threshold_OC_{numero_ocupacao}.txt"
