...
# Objetivo do código: geração de arquivos de saída baseados nos dados estatísticos dos histogramas do erro de estimação pelo método de Desconvolução de Sinal Versão Threshold.
...
Histograma do erro da estimação da amplitude pelo método de Desconvolução Versão Threshold: histograma_erro_amplitude_desconvolucao_threshold.py

Funções presentes:

1) Função para o cálculo dos dados estatístico do erro de estimação da amplitude pelo método de Desconvolução de Sinal Versão Threshold.
Entrada: lista com o erro de estimação da amplitude.
Saída: a média, a variância e o desvio padrão do erro de estimação.

2) Instrução para salvar os dados estatísticos do erro de estimação da amplitude para determinada ocupação em um arquivo de saída pelo método de Desconvolução de Sinal Versão Threshold.
...
titulo_programa = colored("Geração de arquivos de saída baseados nos dados estatísticos dos histogramas do erro de estimação pelo método de Desconvolução de Sinal Versão Threshold:\n", attrs=["bold"])
...
def arquivo_saida_dados_estatisticos_erro_estimacao_amplitude_desconvolucao_threshold(n_ocupacao, n_janelamento, media_erro_estimacao_amplitude, var_erro_estimacao_amplitude, desvio_padrao_erro_estimacao_amplitude):
    ...
    pasta_saida = "Dados_Estatisticos_Desconvolucao_Threshold_OC"
    ...
    arquivo_saida = f"dados_estatisticos_desconvolucao_threshold_janelamento_{n_janelamento}.txt"
...
def principal_arquivo_saida_dados_estatisticos_desconvolucao_threshold():
    ...
    for n_janelamento in tqdm(range(n_janelamento_inicial, n_janelamento_final+1, incremento_janelamento)):
        ...
        for numero_ocupacao in tqdm(range(ocupacao_inicial, ocupacao_final+1, incremento_ocupacao)):
            ...
            lista_erro_amplitude = metodo_desconvolucao_threshold_P_igual_N(n_janelamento, Matriz_Dados_Pulsos_Janelado, vetor_parametro_referencia_janelado)
